<table id="timetable_class">
    <?php
        ...
        $name_dis = mysqli_query($conn, "SELECT * FROM timetable_class WHERE weekday = '$we' order by number_para");
                ...
                if (!empty($data)){
                    ...
                    for ($j = 0; $j < count($data); $j++){
                        if ($arr[$i] == $data[$j]["number_para"]){
                            ...
                            echo $data[$j]['type_para'].'</td><td>'.$diss['name_discipline'] . '<br>' . $off . '<br> (' . $data[$j]['group_kurs'] . ')<br>'. $prof['FIO'].'<br>';
                        }
